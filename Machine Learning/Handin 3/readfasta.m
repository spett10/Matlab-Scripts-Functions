function [data, seq] = readfasta(filename,varargin)
%READFASTA reads FASTA format file.
%
%   S = READFASTA(FILENAME) reads a FASTA format file FILENAME, returning
...
%
%   [HEADER, SEQ] = READFASTA(FILENAME) reads the file into separate
...
%
%   READFASTA(...,'IGNOREGAPS',TF) removes any gap symbol ('-' or '.')
%   from the sequence(s) when TF is true. Default is false.
%
%   READFASTA(...,'BLOCKREAD', M) allows you to read in a single entry or
...
%
%   READFASTA(...,'TRIMHEADERS',TF) trims the header after the first
...
%       p53nt = readfasta('p53nt.txt')
%
%       % Read the sequence for the human p53 tumor protein.
%       p53aa = readfasta('p53aa.txt')
%
%       % Read a block of entries from a file
%       pf2_5_10 = readfasta('pf00002.fa','blockread',[ 5 10], ...
%                            'ignoregaps',true)
%
...
%       mitochondrion = readfasta([entrezSite textOptions genbankID])
%
...
if ~ischar(filename)
    error('Bioinfo:readfasta:InvalidInput','Input must be a character array')
end
...
if  nargin > 1
    if rem(nargin,2) ~= 1
        error('Bioinfo:readfasta:IncorrectNumberOfArguments',...
    ...
    for j=1:2:nargin-1
        ...
        if isempty(k)
            error('Bioinfo:readfasta:UnknownParameterName',...
                'Unknown parameter name: %s.',pname);
            %elseif length(k)>1
            %    error('Bioinfo:readfasta:AmbiguousParameterName',...
            ...
            switch(k)
                case 1  % ignore gaps
                    ...
                    if isempty(ignoreGaps)
                        error('Bioinfo:readfasta:IgnoreGapsNotLogical',...
                ...
                case 2  % range
                    ...
                    if ~isnumeric(range) || numel(range)> 2 || isempty(range)
                        error('Bioinfo:readfasta:BadBlockRange',...
...
if size(filename,1)>1  % is padded string
    if blockRead
        warning('Bioinfo:readfasta:IgnoredRange',...
...
elseif (strfind(filename(1:min(10,end)), '://'))
    ...
    if (~usejava('jvm'))
        error('Bioinfo:readfasta:NoJava',...
            'Reading from a URL requires Java.')
    end
    try
        ftext = urlread(filename);
    catch allExceptions
        error('Bioinfo:readfasta:CannotReadURL',...
    ...
    if blockRead
        ...
        try
           ...
           if strcmpi(theErr.identifier,'MATLAB:nomem')
                error( 'Bioinfo:readfasta:BlockTooBig',...
        ...
        try
            ...
            if strcmpi(theErr.identifier,'MATLAB:nomem')
                error( 'Bioinfo:readfasta:FileTooBig',...
...
if ~any(commentLines)
    error('Bioinfo:readfasta:FastaNotValid',...
...
try
    ...
    error('Bioinfo:readfasta:IncorrectDataFormat',...
