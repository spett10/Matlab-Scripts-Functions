...
plot(0.5+0.3*s+3.9*s.^2-4.7*s.^3, 1.5+0.3*s+0.9*s.^2-2.7*s.^3) 
%plot the path
ball=line('color','r','Marker','o','MarkerSize',10,'LineWidth',2,'erase','xor','xdata',[],'ydata',[]);
%define our ball as in the book
t=zeros(20,1); %we define values for our t, so we can compute x and y values
for i=0:20
[ a b ]=equpartition(i*0.05);
c=(a+b)./2;
t(i+1)=c; %we need the values for the division of the path in 
%equal lengths
% corresponding to constant speed. 0.05 is 1/20, thus the for loop
end
