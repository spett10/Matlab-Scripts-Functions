function [x] = GMRES_backup(A,b,x0,m)
...
%A=makeA(99,49);
%b=makeb(99,49);
%x0 is arbitrary but for testing the following was used
%result=GMRES_backup(A,b,ones(size(A,1),1),50);
%
%visualization was done like so:
%
%imagesc(reshape(result,99,49)')
...
v=v-2*(W(:,1)'*v)*W(:,1); %foerste V:=P1P2..Pjej, dvs v:=p1*ej
z=A*v;
z=z-2*(W(:,1)'*z)*(W(:,1)); %da 1<=m, z:=P1Av, vores nye z

for j=2:m+1
   %compute householder unit vector w_j
   W(1:j-1,j)=zeros(j-1,1); %foerste j-1 komponenter er 0! (1.25 linje 1)
   ...
   v = zeros(n,1);%v=P1P2..Pjej
			v(j)=1;%e_j
			for k=j:-1:1
				v = v - 2*(W(:,k)'*v)*W(:,k);%vi goer det den anden vej 
            end
   if (j<=m)%linje 8
      z = A*v;%z:=P_jP_j-1...P_1Av
			for t=1:j
				z = z - 2*(W(:,t)'*z)*W(:,t);%vi goer det den anden vej
            end
   end
