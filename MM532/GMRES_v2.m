...
res=g(m+1);
ym=zeros(m,1);
%calculate the vector ym by backwards substitution of the upper triangular
%problem like so:
for i=m:-1:1
    sumin=0;
    for j=i+1:m
...
tempx=x0+z;
if (res>10^-5)%if tolerance not met, restart with x as x0
   %disp('Restarting!');
   x=tempx;
   GMRES(A,b,x,m)
else
   x=tempx; %if below tolerance, return approx x
end
