...
function y=embrung23(t,w,h);
%one step of the embedded Runge-Kutta order 2/3 method 
s1=ydot(t,w);
s2=ydot(t+h,w+h*s1);
s3=ydot(t+0.5*h,w+0.5*h*((s1+s2)/2));
w=w+h*(s1+s2);
z=w+h*((s1+4*s3+s2)/6);
y=w;


function h=alterh(t,w,h);
s1=ydot(t,w);
s2=ydot(t+h,w+h*s1);
s3=ydot(t+0.5*h,w+0.5*h*((s1+s2)/2));
w=w+h*(s1+s2);
